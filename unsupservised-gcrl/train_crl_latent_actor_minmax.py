...
if __name__ == "__main__":
    ...
    init_state_const =  env_state.obs[:, :args.obs_dim]
    ...
    def get_experience(training_state, transitions, env_state, buffer_state, key):    
        ...
        def get_worst_best_goal(rg):
            ...
            actor_params, sa_encoder_params, g_encoder_params = jax.lax.stop_gradient(training_state.actor_state.params), jax.lax.stop_gradient(training_state.critic_state.params["sa_encoder"]), jax.lax.stop_gradient(training_state.critic_state.params["g_encoder"])
            random_g_repr = g_encoder.apply(g_encoder_params, rg) 

            rda_means, _ = actor.apply(actor_params, init_state_const, random_g_repr)
            random_directed_actions = nn.tanh( rda_means )

            random_sa_repr = sa_encoder.apply(sa_encoder_params, init_state_const, random_directed_actions)
                       
            logits = -jnp.sqrt(jnp.sum((random_sa_repr - random_g_repr) ** 2, axis=-1))

            return random_g_repr[ jnp.argmin( logits ) ]
        ...
        explore_goal_reps = jax.vmap(get_worst_best_goal, in_axes=0)(random_goals)
