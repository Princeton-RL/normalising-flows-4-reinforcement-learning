...
from ur3_env import UR3Wrapper
from vqdet_dataset import get_ur3_train_val
...
class Args:
    ...
    dataset_dir: str = '/n/fs/url-rg/vqbet-data/vqbet_datasets_for_release/ur3/'
...
if __name__ == "__main__":
    ...
    # Set up logger.
    exp_name = f"{args.wandb_name_tag + '__' if args.wandb_name_tag is not None else ''}__ur3__fcnf_gcbc__{args.channels}-{args.blocks}-{args.rep_dim}__{args.wandb_group}__{int(time.time())}"
    ...
    train_data, test_data = get_ur3_train_val(
    ...
    def make_ur3_envs(rank: int, seed: int = 0):
        def _init():
            env = UR3Wrapper(
    ...
    envs = DummyVecEnv([make_ur3_envs(i, args.seed) for i in range(args.num_eval_episodes)])
