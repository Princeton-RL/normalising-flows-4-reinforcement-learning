...
from ant_env import ant_mask_targets, AntWrapper
from vqdet_dataset import get_ant_train_val
...
class Args:
    ...
    dataset_dir: str = '/n/fs/url-rg/vqbet-data/vqbet_datasets_for_release/ant/'
    ...
    eval_action_window_size: int = 35
...
if __name__ == "__main__":
    ...
    # Set up logger.
    exp_name = f"{args.wandb_name_tag + '__' if args.wandb_name_tag is not None else ''}__ant__fcnf_gcbc__{args.channels}-{args.blocks}-{args.rep_dim}__{args.wandb_group}__{int(time.time())}"
    ...
    train_data, test_data = get_ant_train_val(
    ...
    def make_ant_envs(rank: int, seed: int = 0):
        def _init():
            env = AntWrapper(
                env = gym.make('AntMazeMultimodal-eval-v0'),
                goal_cond = False,
    ...
    envs = DummyVecEnv([make_ant_envs(i, args.seed) for i in range(args.num_eval_episodes)])
    ...
    def evaluate_parallel_bc():
        ...
        while not np.all(done):
            ...
            if sample_new_action_count == args.eval_action_window_size:
                ...
                obs = normalize_data(obs, stats['obs']).to(device, dtype=torch.float32)
                ...
                action = unnormalize_data(action.to('cpu'), stats['actions']).numpy()
